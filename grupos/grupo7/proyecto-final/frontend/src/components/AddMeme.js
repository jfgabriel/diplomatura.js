    ...
    return (
      ...
      >
        {({ imageList, onImageUpload, onImageUpdate, onImageRemove,dragProps }) => (
          <div>
            <div>Titulo :
              <input type="text" name="titulo" value={titulo} onChange={handleTituloChange}/>
            </div>
            {imageList.length===0 && <div><button onClick={onImageUpload} {...dragProps}>Buscar Imagen</button></div>}
            ...
            <select name="categotias" className="mdb-select md-form" value={categoria} onChange={handleCategoriaChange}>
                  {categorias.map((cat) => (
            ...
            <div>
                <button type="submit" onClick={handleGuardarOnClick}>Guardar</button>
            </div>
          </div>
        )}
      </ImageUploading>
